...
const isNil = d => !d || d === 'null' || d === 'undefined';
const pD = d => { if(isNil(d)) return null; const s = String(d); const dt = new Date(s.endsWith('Z') || s.includes('+') ? s : s + 'Z'); return isNaN(dt.getTime()) ? null : dt; };
const fmt = d => { if(isNil(d)) return null; const dt = pD(d); if(!dt) return null; try { return dt.toLocaleDateString('en-IN',{day:'numeric',month:'short',year:'numeric'}); } catch { return null; } };
const fmtDt = d => { if(isNil(d)) return ''; const dt = pD(d); if(!dt) return ''; try { return dt.toLocaleString('en-IN',{day:'numeric',month:'short',hour:'2-digit',minute:'2-digit'}); } catch { return ''; } };
const fmtRel = d => { if(isNil(d)) return ''; const dt = pD(d); if(!dt) return ''; const diff = Date.now()-dt.getTime(); const m=Math.floor(diff/60000),h=Math.floor(diff/3600000),dy=Math.floor(diff/86400000); if(m<1) return 'just now'; if(m<60) return m+'m ago'; if(h<24) return h+'h ago'; if(dy<7) return dy+'d ago'; return fmt(d)||''; };
const drN = n => { if(!n || n==='null') return 'Unknown'; return n.match(/^dr\.?\s/i) ? n : 'Dr. ' + n; };
const dateVal = d => { if(isNil(d)) return ''; const s = String(d); const m = s.match(/(\d{4}-\d{2}-\d{2})/); return m ? m[1] : ''; };
...
function RecordModal({record,profileId,allRecords,onClose,onDeleted,onUpdated,onOpenRecord}) {
  ...
  const [form,sForm]=useState({document_type:record.document_type||'',document_category:record.document_category||'prescription',doctor_name:record.doctor_name||'',hospital_name:record.hospital_name||'',document_date:dateVal(record.document_date),specialty:record.specialty||'',diagnosis:record.diagnosis||'',recommendations:record.recommendations||'',bill_amount:record.bill_amount!=null?String(record.bill_amount):''});
  useEffect(()=>{sTab('details');sEditing(false);sHld(false);sGal(null);sForm({document_type:record.document_type||'',document_category:record.document_category||'prescription',doctor_name:record.doctor_name||'',hospital_name:record.hospital_name||'',document_date:dateVal(record.document_date),specialty:record.specialty||'',diagnosis:record.diagnosis||'',recommendations:record.recommendations||'',bill_amount:record.bill_amount!=null?String(record.bill_amount):''});},[record.id,record.document_type,record.document_category,record.doctor_name,record.hospital_name,record.document_date,record.specialty,record.diagnosis,record.recommendations,record.bill_amount]);
  ...
  const doSave=async()=>{sSaving(true);try{const p={};Object.entries(form).forEach(([k,v])=>{if(k==='bill_amount'){if(v!=='')p[k]=parseFloat(v);}else if(v!=='')p[k]=v;});if(form.document_category)p.document_category=form.document_category;const r=await API.put('/profiles/'+profileId+'/records/'+record.id,p);onUpdated(r.data);sEditing(false);sHld(false);}catch(e){alert('Save failed');}finally{sSaving(false);}};
...
function DoctorCard({doctor,visits,onOpenRecord,profileId,onUploadFor}) {
  ...
  return <div className="dcard">
    ...
    <div className="dcard-chips">{visits.slice(0,8).map(v=><div key={v.id} className={"dchip"+(expanded===v.id?' active':'')} onClick={()=>{setExpanded(expanded===v.id?null:v.id);sVtab('details');}}>{(!doctor||doctor==='Unassigned')?(v.document_category==='bill'?'Bill':'Doc')+' '+(fmt(v.document_date)||fmtRel(v.created_at)):(fmt(v.document_date)||fmt(v.created_at))}</div>)}{visits.length>8&&<div className="dchip" style={{color:'#94a3b8',cursor:'default'}}>+{visits.length-8} more</div>}</div>
...
function Dashboard({onLogout}) {
  ...
  const normDr=n=>(n||'').toLowerCase().replace(/^dr\.?\s*/i,'').replace(/\s*\(.*?\)\s*/g,' ').replace(/\s+/g,' ').trim();
  const docGroups={};const docNameMap={};records.filter(r=>r.status==='done'||r.status==='extracting').forEach(r=>{const raw=r.doctor_name||'Unassigned';const key=normDr(raw)||'unassigned';if(!docNameMap[key])docNameMap[key]=raw;if(!docGroups[key])docGroups[key]=[];docGroups[key].push(r);});Object.values(docGroups).forEach(a=>a.sort((a,b)=>new Date(b.created_at)-new Date(a.created_at)));
