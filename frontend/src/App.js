...
function RecordModal({record,profileId,allRecords,onClose,onDeleted,onUpdated,onOpenRecord}) {
  ...
  {tab==='details'&&!editing&&<div>
    <div className="drow"><div className="drow-icon" style={{background:'#fffbeb'}}></div><div style={{flex:1}}><div className="drow-key">Date</div>{record.document_date?<div className="drow-val">{fmt(record.document_date)}</div>:<div style={{display:'flex',alignItems:'center',gap:8}}><input type="date" className="edit-inp" style={{maxWidth:180,padding:'6px 10px',fontSize:12}} onChange={async e=>{if(!e.target.value)return;try{const r=await API.put('/profiles/'+profileId+'/records/'+record.id,{document_date:e.target.value});onUpdated(r.data);}catch(ex){}}}/><span style={{fontSize:11,color:'#94a3b8'}}>Not set — pick a date</span></div>}</div></div>
    ...
    <div className="app">
      ...
      <div className="page">
        ...
        {!loading&&sel&&page==='home'&&<>
          ...
          {sortedDocs.length>0&&<><div className="sec-title">My Doctors</div><div className="doc-grid">{sortedDocs.map(key=><DoctorCard key={key} doctor={docNameMap[key]} visits={docGroups[key]} onOpenRecord={r=>setSelRec(r)} profileId={sel?.id} onUploadFor={(e,type)=>onFileSelect(e,type)}/>)}</div></>}
