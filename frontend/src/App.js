...
const css = `
  ...
  *{box-sizing:border-box;margin:0;padding:0;}
  ...
  @media(min-width:600px){.upload-preview{align-items:center;}}
  ...
  @media(min-width:600px){.upload-preview-box{border-radius:16px;}}
  ...
  .bills-page{padding:0;}
  .bills-summary{display:grid;grid-template-columns:1fr 1fr 1fr;gap:10px;margin-bottom:20px;}
  .bs-card{background:#fff;border-radius:14px;padding:16px;text-align:center;box-shadow:0 2px 8px rgba(0,0,0,0.05);}
  .bs-val{font-size:22px;font-weight:700;color:#0f172a;letter-spacing:-0.5px;}
  .bs-lbl{font-size:11px;color:#64748b;margin-top:2px;font-weight:500;}
  .month-group{margin-bottom:20px;}
  .month-header{font-size:14px;font-weight:700;color:#0f172a;margin-bottom:10px;display:flex;justify-content:space-between;align-items:center;}
  .month-total{color:#0d9488;font-size:13px;}
  .bill-item{background:#fff;border-radius:14px;padding:14px 16px;margin-bottom:8px;box-shadow:0 1px 4px rgba(0,0,0,0.04);border:1px solid #e2e8f0;display:flex;align-items:center;gap:12px;}
  .bill-info{flex:1;min-width:0;}
  .bill-doctor{font-size:14px;font-weight:600;color:#0f172a;}
  .bill-hospital{font-size:12px;color:#94a3b8;}
  .bill-right{text-align:right;flex-shrink:0;}
  .bill-amount{font-size:16px;font-weight:700;color:#0f172a;}
  .bill-date{font-size:11px;color:#94a3b8;}
  ...
  .doc-thumbs{display:flex;gap:8px;margin-bottom:12px;flex-wrap:wrap;}
  .doc-thumb{width:90px;height:70px;border-radius:10px;border:1px solid #e2e8f0;overflow:hidden;cursor:pointer;position:relative;background:#f1f5f9;}
  .doc-thumb img{width:100%;height:100%;object-fit:cover;}
  .doc-thumb-label{position:absolute;bottom:0;left:0;right:0;background:rgba(0,0,0,0.6);color:#fff;font-size:9px;font-weight:700;padding:2px 4px;text-align:center;}
  @media(max-width:768px){
    .bills-summary{grid-template-columns:1fr 1fr 1fr;gap:8px;}
    .bs-val{font-size:18px;}
  }
  @media(max-width:380px){
...
const MONTH_NAMES_L = ['January','February','March','April','May','June','July','August','September','October','November','December'];
const fmtMonth = key => { if(!key || key==='Unknown') return 'Unknown'; const [y,m]=key.split('-'); return MONTH_NAMES_L[parseInt(m,10)-1]+' '+y; };
const drName = n => { if(!n) return 'Unknown'; return n.match(/^dr\.?\s/i) ? n : 'Dr. ' + n; };
...
function ImageGallery({files, startIndex, onClose}) {
  ...
  return (
    <div className="gallery-overlay" onClick={onClose}>
      ...
      <img className="gallery-img" src={files[idx].file_url} alt={'Page '+(idx+1)} onClick={e => e.stopPropagation()} />
    ...
    <div className="upload-preview">
      <div className="upload-preview-box" onClick={e => e.stopPropagation()}>
        ...
        <div className="up-actions">
          <label className="up-btn up-btn-add" style={{display:'flex',alignItems:'center',justifyContent:'center',gap:6,cursor:'pointer'}}>
            + Add Pages
...
function BillsPage({profileId, showToast}) {
  ...
  useEffect(() => {
    if(!profileId) return;
    setLoading(true);
    API.get('/profiles/' + profileId + '/bills')
  ...
  const toggleInsurance = async (recordId) => {
    try {
      const r = await API.put('/profiles/' + profileId + '/records/' + recordId + '/insurance');
      setData(prev => {
        if(!prev) return prev;
        ...
        return { ...prev, months: newMonths, summary: {...prev.summary, total_claimed: totalClaimed, unclaimed: prev.summary.total_spent - totalClaimed} };
      });
    } catch(e) { showToast('Failed to toggle insurance', 'error'); }
  };
  ...
  return (
    <div className="bills-page">
      <div className="bills-summary">
        <div className="bs-card"><div className="bs-val">{fmtCurrency(data.summary.total_spent)}</div><div className="bs-lbl">Total Spent</div></div>
        <div className="bs-card"><div className="bs-val" style={{color:'#10b981'}}>{fmtCurrency(data.summary.total_claimed)}</div><div className="bs-lbl">Claimed</div></div>
        <div className="bs-card"><div className="bs-val" style={{color:'#ef4444'}}>{fmtCurrency(data.summary.unclaimed)}</div><div className="bs-lbl">Unclaimed</div></div>
      </div>
      {data.months.length === 0 && (
        <div className="empty"><div className="empty-icon"><svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="#94a3b8" strokeWidth="1.5"><rect x="2" y="5" width="20" height="14" rx="2"/><line x1="2" y1="10" x2="22" y2="10"/></svg></div><div className="empty-title">No bills yet</div><div className="empty-sub">Upload a medical bill to track expenses.</div></div>
      )}
      {data.months.map(month => (
        <div key={month.month} className="month-group">
          <div className="month-header"><span>{fmtMonth(month.month)}</span><span className="month-total">{fmtCurrency(month.total)}</span></div>
          {month.bills.map(bill => {
            const files = bill.files || (bill.file_url ? [{file_url: bill.file_url, page_number: 1}] : []);
            return (
              <div key={bill.id} className="bill-item">
                <div className="bill-info" style={{cursor: files.length > 0 ? 'pointer' : 'default'}} onClick={() => files.length > 0 && setGalleryFiles(files)}>
                ...
                <div className="bill-right"><div className="bill-amount">{fmtCurrency(bill.bill_amount)}</div><div className="bill-date">{fmt(bill.document_date) || fmtRel(bill.created_at)}</div></div>
      ...
      {galleryFiles && <ImageGallery files={galleryFiles} startIndex={0} onClose={() => setGalleryFiles(null)} />}
    </div>
  );
}
    ...
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal" onClick={e => e.stopPropagation()}>
        ...
        <div className="modal-body">
          ...
          {tab === 'ocr' && (() => {
            const docFiles = record.files && record.files.length > 0 ? record.files : (record.file_url ? [{file_url: record.file_url, file_path: record.file_path, page_number: 1}] : []);
            return (
            <div>
              {docFiles.length > 0 && (
                <div className="doc-thumbs">
                  {docFiles.map((f, i) => (
                    <div key={i} className="doc-thumb" onClick={() => setGalleryIdx(i)}>
                      <img src={f.file_url} alt={'Page '+(f.page_number||i+1)} onError={e => e.target.style.display='none'} />
                      <div className="doc-thumb-label">Page {f.page_number||i+1}</div>
                    </div>
                  ))}
                </div>
              )}
              ...
              {galleryIdx !== null && <ImageGallery files={docFiles} startIndex={galleryIdx} onClose={() => setGalleryIdx(null)} />}
            </div>
            );
          })()}
...
function Dashboard({onLogout}) {
  ...
  const handleRemoveFile = idx => {
    setUploadFiles(prev => { const n = prev.filter((_, i) => i !== idx); return n.length === 0 ? null : n; });
  };
  const handleUpload = async () => {
    ...
    try {
      await API.post('/upload/' + sel.id, fd, {headers:{'Content-Type':'multipart/form-data'}, timeout: 60000});
      setUploadFiles(null);
  ...
  const upload = e => { handleFileSelect(e); };
  ...
  return (
    <>
      ...
      {uploadFiles && (
        <UploadPreview files={uploadFiles} onAdd={handleAddMoreFiles} onRemove={handleRemoveFile} onUpload={handleUpload} uploading={uploading} />
      )}
      {selRecord && (
        <RecordModal
          record={selRecord}
      ...
      <div className="app-root">
            ...
            <div className="header-right">
              {sel && (
                <label className="upload-btn">
                  ...
                  Upload
                  <input type="file" hidden multiple onChange={upload} accept="image/*,.pdf" />
                </label>
              )}
        ...
        <div className="page">
          ...
          {sel && activePage === 'bills' && (
            <div className="page-header">
              <div>
                <h1 className="page-title">{sel.name}'s Bills</h1>
                <p className="page-sub">Medical expenses and insurance</p>
              </div>
            </div>
          )}

          {/* Enhanced Dashboard */}
          {sel && records.length > 0 && activePage === 'records' && (
            <>
          ...
          {/* OCR slow warnings */}
          {Object.keys(ocrSlowWarning).length > 0 && activePage === 'records' && (
          ...
          {searchResults !== null && activePage === 'records' && (
          ...
          {sel && loading && activePage === 'records' && <SkeletonCards />}

          {sel && !loading && display.length === 0 && !uploading && viewMode === 'cards' && activePage === 'records' && (
            <div className="empty">
              <div className="empty-icon">
                {searchResults !== null
          ...
          {sel && !loading && viewMode === 'cards' && activePage === 'records' && (
          ...
          {sel && !loading && viewMode === 'doctors' && activePage === 'records' && (
          ...
          {sel && activePage === 'bills' && (
            <BillsPage profileId={sel.id} showToast={showToast} />
          )}
        </div>

        <nav className="bottom-nav">
          <button className={"bnav-item" + (activePage === 'records' ? ' active' : '')} onClick={() => { setActivePage('records'); window.scrollTo({top:0,behavior:'smooth'}); }}>
          ...
          <button className={"bnav-item" + (activePage === 'search' ? ' active' : '')} onClick={() => { setActivePage('records'); setTimeout(() => { if(searchRef.current) searchRef.current.focus(); }, 100); }}>
          ...
          <button className={"bnav-item" + (activePage === 'bills' ? ' active' : '')} onClick={() => setActivePage('bills')}>
