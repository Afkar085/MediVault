...
function RecordModal({record,profileId,allRecords,onClose,onDeleted,onUpdated,onOpenRecord}) {
  ...
  const [form,sForm]=useState({document_type:record.document_type||'',doctor_name:record.doctor_name||'',hospital_name:record.hospital_name||'',document_date:record.document_date||'',specialty:record.specialty||'',diagnosis:record.diagnosis||'',recommendations:record.recommendations||''});
  useEffect(()=>{sTab('details');sEditing(false);sHld(false);sGal(null);sForm({document_type:record.document_type||'',doctor_name:record.doctor_name||'',hospital_name:record.hospital_name||'',document_date:record.document_date||'',specialty:record.specialty||'',diagnosis:record.diagnosis||'',recommendations:record.recommendations||''});},[record.id,record.document_type,record.doctor_name,record.hospital_name,record.document_date,record.specialty,record.diagnosis,record.recommendations]);
  ...
  const doSave=async()=>{sSaving(true);try{const p={};Object.entries(form).forEach(([k,v])=>{if(v!=='')p[k]=v;});const r=await API.put('/profiles/'+profileId+'/records/'+record.id,p);onUpdated(r.data);sEditing(false);sHld(false);}catch(e){alert('Save failed');}finally{sSaving(false);}};
...
function Dashboard({onLogout}) {
  ...
  const docGroups={};records.filter(r=>r.status==='done'||r.status==='extracting').forEach(r=>{const k=r.doctor_name||'Unassigned';if(!docGroups[k])docGroups[k]=[];docGroups[k].push(r);});Object.values(docGroups).forEach(a=>a.sort((a,b)=>new Date(b.created_at)-new Date(a.created_at)));
  ...
  return <>
    ...
    <div className="app">
      ...
      <div className="page">
        ...
        {!loading&&sel&&page==='bills'&&<><div style={{marginBottom:16}}><h2 style={{fontSize:20,fontWeight:700,color:'#0f172a',letterSpacing:'-0.02em'}}>{sel.name}'s Bills</h2><p style={{fontSize:13,color:'#64748b',marginTop:2}}>Medical expenses & insurance</p></div><BillsPage profileId={sel.id} showToast={showToast}/></>}
